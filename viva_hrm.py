...
# ==============================================================================
VIVA_SUBJECT = "Human Resource Management"      # LOCKED subject for this deployment
...
<head>
    ...
    // Without componentReady, Streamlit silently DROPS setComponentValue
    // messages, so 'cheated'/'timeout' signals never reach Python even though
    // the timer keeps ticking on screen.
    ...
    try {{
        guardDoc.addEventListener('visibilitychange', function() {{
            if (guardDoc.visibilityState === 'hidden') reportCheat("tab hidden / minimized (top)");
        }});
    }} catch (e) {{}}

    // 2) Window lost focus -> another window OR app/file opened on top.
    ...
    // 3) block right-click, 4) block copy/cut/paste
    try {{
    ...
    // 5) block new-window / new-tab / dev-tools shortcuts
    function keyGuard(e) {{
    ...
    // 6) warn before closing / reloading
